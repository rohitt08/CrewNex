...
import { Loader2, AlertCircle, CheckCircle2, Lock, FileText, BrainCircuit, Timer, HelpCircle, User, ArrowRight, Play, Check } from "lucide-react";
...
const GradientCard = ({ children, className = "" }) => (
  <div className={`glass-card border border-white/40 rounded-[2rem] shadow-2xl overflow-hidden relative ${className}`}>
...
const Header = () => (
  <div className="bg-gradient-premium px-8 py-6 text-center border-b border-indigo-500/20 relative overflow-hidden">
     ...
     <div className="relative z-10 flex items-center justify-center gap-3">
        ...
        <div>
          <span className="text-2xl font-black text-white tracking-tight leading-none block">CrewNex</span>
          <p className="text-indigo-200 text-[10px] mt-0.5 font-bold uppercase tracking-widest">Online Assessment Portal</p>
        </div>
     </div>
  </div>
);
      ...
      <div className="w-full max-w-2xl relative z-10 animate-fade-in-up">
        ...
        {stage === STAGE.VERIFYING && (
          <GradientCard>
            ...
            <div className="p-12 text-center">
              ...
              <h2 className="text-2xl font-black text-slate-900 mb-3">Verifying Secure Link</h2>
              <p className="text-slate-500 text-base font-medium">Please wait while we validate your assessment invitation...</p>
            </div>
          </GradientCard>
        )}
        ...
        {stage === STAGE.ERROR && (
          <GradientCard>
            ...
            <div className="p-12 text-center">
              ...
              <h2 className="text-2xl font-black text-red-600 mb-4">Link Invalid or Expired</h2>
              <p className="text-slate-500 text-base font-medium leading-relaxed mb-8">{errorMsg}</p>
        ...
        {stage === STAGE.LOGIN && (
          <GradientCard>
            ...
            <div className="bg-indigo-50/50 border-b border-indigo-100/50 px-8 py-6 text-center">
              <p className="text-base font-semibold text-slate-700">You have been shortlisted for <span className="font-extrabold text-slate-900">{info?.projectTitle}</span></p>
              <p className="text-sm font-medium text-slate-500 mt-1">Please sign in to access the assessment for the <strong className="text-indigo-600">{info?.roleName}</strong> role</p>
            </div>
            <div className="p-8 sm:p-10">
              <h2 className="text-2xl font-black text-slate-900 mb-2">Sign In to Continue</h2>
              <p className="text-sm font-medium text-slate-500 mb-8">Use your CrewNex account credentials</p>

              <form onSubmit={handleLogin} className="space-y-5">
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2"><User className="w-3.5 h-3.5"/> Email Address</label>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2"><Lock className="w-3.5 h-3.5"/> Password</label>
                ...
                <button type="submit" disabled={loginLoading}
                  ...
                  ) : <><Lock className="w-4 h-4"/> Sign In & Continue <ArrowRight className="w-4 h-4"/></>}
                </button>
              </form>
            </div>
          </GradientCard>
        ...
        {stage === STAGE.BRIEFING && (
          <GradientCard>
            <Header />
            <div className="bg-indigo-50/50 border-b border-indigo-100/50 px-8 py-6 text-center">
              <p className="text-base font-semibold text-slate-700">Assessment for <strong className="text-indigo-600 text-lg">{info?.roleName}</strong></p>
              <p className="text-sm font-medium text-slate-500 mt-1">Project: <strong className="text-slate-900">{info?.projectTitle}</strong></p>
            </div>

            <div className="p-8 sm:p-10">
              <h2 className="text-2xl font-black text-slate-900 mb-2">Assessment Instructions</h2>
              <p className="text-sm font-medium text-slate-500 mb-8">Please read carefully before starting</p>

              <div className="grid grid-cols-2 gap-4 mb-8">
                {[
                  { icon: HelpCircle, label: "Questions", val: "30 MCQs", color: "text-blue-600", bg: "bg-blue-50" },
                  { icon: Timer, label: "Duration", val: "15 Minutes", color: "text-orange-600", bg: "bg-orange-50" },
                  { icon: BrainCircuit, label: "Assessment", val: "Skills Based", color: "text-purple-600", bg: "bg-purple-50" },
                  { icon: CheckCircle2, label: "Grading", val: "Auto-Graded", color: "text-emerald-600", bg: "bg-emerald-50" },
                ].map((s) => (
                  <div key={s.label} className="bg-white border border-slate-100 rounded-2xl p-5 flex items-center gap-4 shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="bg-amber-50 border border-amber-200/60 rounded-2xl p-6 mb-8 shadow-sm">
                <p className="text-xs font-bold text-amber-700 uppercase tracking-widest mb-3 flex items-center gap-2"><AlertCircle className="w-4 h-4"/> Important Guidelines</p>
                <ul className="space-y-2 text-sm font-medium text-amber-800/80">
                  <li className="flex items-start gap-2"><span className="text-amber-500">•</span> The timer starts immediately when you click Start.</li>
                  <li className="flex items-start gap-2"><span className="text-amber-500">•</span> The test will auto-submit when time runs out.</li>
                  <li className="flex items-start gap-2"><span className="text-amber-500">•</span> You can only take this assessment once.</li>
                </ul>
              </div>
        ...
        {stage === STAGE.TEST && (
          <div className="space-y-6">
              ...
              <GradientCard className="!rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
                <div className="px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-0.5">Test in Progress</p>
                      <p className="text-sm font-extrabold text-slate-800">{info?.roleName}</p>
                    </div>
                  </div>
                  <div className="flex items-center justify-between sm:justify-end gap-5">
                    <div className="text-right">
                       <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Progress</p>
                       <p className="text-sm font-extrabold text-slate-700">{answered} / {questions.length}</p>
                    </div>
                    <div className={`px-5 py-2.5 rounded-xl font-mono font-black text-xl shadow-inner border flex items-center gap-2 ${
                      timerDanger ? "bg-red-50 text-red-600 border-red-200 animate-pulse" :
            ...
            <div className="space-y-5 pb-10">
              {questions.map((q, qi) => (
                <div key={q.id || qi} className="glass-card border border-white/40 rounded-3xl p-6 sm:p-8 shadow-sm">
                  <div className="flex flex-col sm:flex-row sm:items-start gap-4 mb-6">
                    <div className={`shrink-0 w-10 h-10 rounded-xl flex items-center justify-center text-sm font-black shadow-sm ${answers[qi] !== null ? "bg-indigo-600 text-white" : "bg-slate-100 text-slate-500 border border-slate-200"}`}>
                      {qi + 1}
                    </div>
                    <div className="pt-1">
                      {q.skill && (
                    ...
                    {q.options.map((opt, oi) => {
                      ...
                      return (
                          <button key={oi} onClick={() => setAnswers((prev) => { const a = [...prev]; a[qi] = oi; return a; })}
                            ...
                            <span className={`flex items-center justify-center w-6 h-6 rounded-md border text-xs font-extrabold shrink-0 ${selected ? "bg-indigo-500 border-indigo-500 text-white" : "bg-slate-100 border-slate-200 text-slate-500 group-hover:bg-white group-hover:border-indigo-300 group-hover:text-indigo-500"}`}>
                               {["A","B","C","D"][oi]}
                            </span>
              ...
              <div className="glass-card border border-white/40 rounded-3xl p-8 text-center mt-8">
                {answered < questions.length ? (
                  <div className="mb-6 inline-flex items-center gap-2 bg-amber-50 text-amber-700 border border-amber-200 px-4 py-2 rounded-xl text-sm font-bold shadow-sm">
                    <AlertCircle className="w-4 h-4" /> {questions.length - answered} question{questions.length - answered > 1 ? "s" : ""} left unanswered
                  </div>
                ) : (
        ...
        {stage === STAGE.RESULT && (
          <GradientCard className="animate-fade-in-up">
            ...
            <div className="p-10 sm:p-14 text-center">
              ...
              <p className="text-slate-500 text-base font-medium mb-10">Completed for <strong className="text-indigo-600">{info?.roleName}</strong> on <strong className="text-slate-800">{info?.projectTitle}</strong></p>

              <div className="bg-slate-50 border border-slate-200/60 rounded-2xl p-6 sm:p-8 mb-10 shadow-inner">
                <p className="text-sm font-semibold text-slate-600 leading-relaxed">
                  Thank you for taking the assessment. Your results have been securely transmitted to the project creator. 
